...
def normalize_augment(datain, ctrl_fr_idx=2):
    ...
    img_train = img_train.view(
        img_train.size()[0], -1, img_train.size()[-2], img_train.size()[-1]
    )
...
def open_image(
    fpath, gray_mode, expand_if_needed=False, expand_axis0=True, normalize_data=True
):
...
def batch_psnr(img, imclean, data_range):
    ...
    for i in range(img_cpu.shape[0]):
        psnr += compare_psnr(
            imgclean[i, :, :, :], img_cpu[i, :, :, :], data_range=data_range
        )
