...
def denoise_video_sequence(noisy_vid, vid_name, sigma, 
                          s_cnn=None, t_cnn=None,  # modified to take pre-loaded models rather then every time
                          clipped_noise=False, gpu_usage=0, silent=False):
    """
    Modified to accept pre-loaded models instead of loading them each time
    """
    
    # Use pre-loaded models instead of creating new ones
    if s_cnn is None or t_cnn is None:
        raise ValueError("Pre-loaded s_cnn and t_cnn models must be provided")
